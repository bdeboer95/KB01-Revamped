...
BOOL CMesh::Load( LPDIRECT3DDEVICE9 pDevice, char* file )
{
	...
	wchar_t wtext[20];
	mbstowcs(wtext, file, strlen(file) + 1);//Plus null
	LPWSTR ptr = wtext;
    HRESULT hr = D3DXLoadMeshFromX( ptr, D3DXMESH_MANAGED, pDevice, NULL, &pMaterialBuffer, NULL, &m_numMaterials, &m_pMesh );
    if ( FAILED( hr ) )
    {
    ...
    for ( DWORD i = 0; i < m_numMaterials; i++ )
    {
        ...
        if( pMaterials[i].pTextureFilename )
        {
			...
			LPWSTR ptr = wtext;
            if ( FAILED( D3DXCreateTextureFromFile( pDevice, ptr, &m_ppMeshTextures[i] ) ) )
            {
